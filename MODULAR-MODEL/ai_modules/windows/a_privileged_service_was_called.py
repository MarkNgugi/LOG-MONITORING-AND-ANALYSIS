...
def detect_privileged_service_call():
    """
    Detects events where a privileged service was called (Event ID 4673) and creates alerts.
    """
    alerts = []
        
    privileged_service_calls = WindowsLog.objects.filter(event_id=4673)
    
    for log in privileged_service_calls:
        try:
            timestamp = log.timestamp
            
            alert = {
                "alert_title": "Privileged Service Call Detected",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"User '{log.log_user}' called a privileged service.",
                "severity": "Medium",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_privileged_service_call()
    ...
    if detected_alerts:
        ...
        print("No privileged service call alerts detected.")
