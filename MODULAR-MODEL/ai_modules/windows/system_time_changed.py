...
def detect_system_time_changed():
    """
    Detects system time change events (Event ID 4616) and creates alerts.
    """
    alerts = []
        
    time_change_events = WindowsLog.objects.filter(event_id=4616)
    
    for log in time_change_events:
        try:
            ...
            alert = {
                ...
                "message": f"System time was changed on the machine '{log.computer}'.",
                "severity": "Medium",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_system_time_changed()
    ...
    if detected_alerts:
        ...
        print("No system time change alerts detected.")
