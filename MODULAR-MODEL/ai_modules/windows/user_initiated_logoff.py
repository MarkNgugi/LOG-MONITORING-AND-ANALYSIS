...
def detect_user_initiated_logoff():
    """
    Detects user-initiated logoff events (Event ID 4647) and creates alerts.
    """
    alerts = []
        
    logoff_events = WindowsLog.objects.filter(event_id=4647)
    ...
    for log in logoff_events:
        try:
            ...
            alert = {
                "alert_title": "User Initiated Logoff Detected",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"User '{log.log_user}' initiated a logoff from the system.",
                "severity": "Low",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_user_initiated_logoff()
    ...
    if detected_alerts:
        ...
        print("No user initiated logoff alerts detected.")
