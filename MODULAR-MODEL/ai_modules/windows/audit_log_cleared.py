...
def detect_audit_log_cleared():
    """
    Detects audit log cleared events (Event ID 1102) and creates alerts.
    """
    alerts = []
        
    audit_log_cleared_events = WindowsLog.objects.filter(event_id=1102)
    
    for log in audit_log_cleared_events:
        try:
            timestamp = log.timestamp
            
            alert = {
                "alert_title": "Audit Log Cleared Detected",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"The audit log was cleared on the system by user '{log.log_user}'. This could indicate tampering or malicious activity.",
                "severity": "Critical",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_audit_log_cleared()
    ...
    if detected_alerts:
        ...
        print("No audit log cleared alerts detected.")
