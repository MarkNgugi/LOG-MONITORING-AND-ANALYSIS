...
def detect_event_log_service_start():
    """
    Detects the Event Log Service Start event (Event ID 6005) and creates alerts.
    """
    alerts = []
        
    event_log_service_start = WindowsLog.objects.filter(event_id=6005)
    
    for log in event_log_service_start:
        try:
            timestamp = log.timestamp
            
            alert = {
                "alert_title": "Event Log Service Started",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"Event Log Service started on system '{log.computer}'. This typically occurs after a system startup or reboot.",
                "severity": "Low",  
                "user": log.log_user,  
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_event_log_service_start()
    ...
    if detected_alerts:
        ...
        print("No Event Log Service start alerts detected.")
