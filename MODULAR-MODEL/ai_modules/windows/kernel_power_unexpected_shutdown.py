...
def detect_kernel_power_shutdown():
    """
    Detects Kernel Power (Unexpected Shutdown) events (Event ID 41) and creates alerts.
    """
    alerts = []
        
    kernel_power_shutdowns = WindowsLog.objects.filter(event_id=41)
    
    for log in kernel_power_shutdowns:
        try:
            timestamp = log.timestamp
            
            alert = {
                "alert_title": "Kernel Power (Unexpected Shutdown) Detected",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"System experienced an unexpected shutdown. Event ID: {log.event_id}",
                "severity": "High",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_kernel_power_shutdown()
    ...
    if detected_alerts:
        ...
        print("No Kernel Power (Unexpected Shutdown) alerts detected.")
