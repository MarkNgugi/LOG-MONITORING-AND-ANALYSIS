...
def detect_privileged_operation():
    """
    Detects operations attempted on privileged objects (Event ID 4674) and creates alerts.
    """
    alerts = []
        
    privileged_operations = WindowsLog.objects.filter(event_id=4674)
    
    for log in privileged_operations:
        try:
            timestamp = log.timestamp
            
            alert = {
                "alert_title": "Privileged Operation Attempt Detected",
                "timestamp": timestamp,
                "hostname": log.computer,
                "message": f"User '{log.log_user}' attempted an operation on a privileged object.",
                "severity": "Medium",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_privileged_operation()
    ...
    if detected_alerts:
        ...
        print("No privileged operation alerts detected.")
