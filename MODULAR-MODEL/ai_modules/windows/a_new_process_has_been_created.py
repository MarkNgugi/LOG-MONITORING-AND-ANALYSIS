...
def detect_new_process():
    """
    Detects new process creation events (Event ID 4688) and creates alerts.
    """
    alerts = []
        
    new_process_events = WindowsLog.objects.filter(event_id=4688)
    
    for log in new_process_events:
        try:
            timestamp = log.timestamp
            process_name = log.log_user 
            
            alert = {
                "alert_title": "New Process Created",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"A new process '{process_name}' has been created on the system.",
                "severity": "Medium",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":
    ...
    detected_alerts = detect_new_process()
    ...
    if detected_alerts:
        ...
        print("No new process creation alerts detected.")
