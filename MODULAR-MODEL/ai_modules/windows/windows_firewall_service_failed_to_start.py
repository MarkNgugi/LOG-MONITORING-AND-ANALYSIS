...
def detect_firewall_service_failure():
    """
    Detects "Windows Firewall Service Failed to Start" events (Event ID 7024) and creates alerts.
    """
    alerts = []
        
    firewall_failures = WindowsLog.objects.filter(event_id=7024)
    
    for log in firewall_failures:
        try:
            timestamp = log.timestamp
            
            alert = {
                "alert_title": "Windows Firewall Service Failure Detected",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"Windows Firewall service failed to start on '{log.computer}'.",
                "severity": "High",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_firewall_service_failure()
    ...
    if detected_alerts:
        ...
        print("No Windows Firewall service failure alerts detected.")
