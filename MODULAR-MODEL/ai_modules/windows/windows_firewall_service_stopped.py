...
def detect_firewall_service_stopped():
    """
    Detects events where the Windows Firewall Service has been stopped (Event ID 5025) and creates alerts.
    """
    alerts = []
        
    firewall_stopped_events = WindowsLog.objects.filter(event_id=5025)
    
    for log in firewall_stopped_events:
        try:
...
if __name__ == "__main__":    
    detected_alerts = detect_firewall_service_stopped()
    ...
    if detected_alerts:
        ...
        print("No 'Windows Firewall Service Stopped' alerts detected.")
