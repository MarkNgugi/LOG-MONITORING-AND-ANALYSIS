...
def detect_object_deletion():
    """
    Detects object deletion events (Event ID 4660) and creates alerts.
    """
    alerts = []
        
    deletion_events = WindowsLog.objects.filter(event_id=4660)
    ...
    for log in deletion_events:
        try:
            ...
            alert = {
                "alert_title": "Object Deletion Detected",
                "timestamp": timestamp,
                "hostname": log.computer,  
                "message": f"An object was deleted by user '{log.log_user}'.",
                "severity": "Medium",  
                "user": log.log_user, 
            }
            alerts.append(alert)
...
if __name__ == "__main__":    
    detected_alerts = detect_object_deletion()
    ...
    if detected_alerts:
        ...
        print("No object deletion alerts detected.")
