...
def detect_new_user_creation(log_lines):
    """
    Detects new user account creation events by checking the service and message fields.
    """
    alerts = []
    ...
    for line in log_lines:
        try:
            ...
            # Check if the service is "useradd" and the message contains "new user"
            if line.service == "useradd" and "new user" in line.message.lower():
                # Extract the username from the message
                match = re.search(r"new user: name=(\w+)", line.message)
                if match:
                    username = match.group(1)
                    
                    # Create an alert for the new user creation
                    alert = {
                        "alert_title": f"New User Account Created: {username}",
                        "timestamp": timestamp,
